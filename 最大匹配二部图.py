# -*- coding: utf-8 -*-
"""
Created on Tue Sep 21 11:58:36 2021
...
class Vertex:
    ...
    def setlabel(self, label):   # 设置标记
        self.labels = label

    def getlabel(self):
        return self.labels

class binpartite_Graph:
    def __init__(self):
...
class Solution():
    binpartiteGraph_dict = {'x1':['y3', 'y5'],
                            'x2':['y1', 'y2', 'y3'],
                            'x3':['y4'],
                            'x4':['y4', 'y5'],
                            'x5':['y5']}
    # 创建二部图
    def createBinpartiteGraph(self, a_dict = binpartiteGraph_dict):
        binpartiteGraph = binpartite_Graph()
        for i in a_dict:
            binpartiteGraph.addVertex(i, 'x')
            for j in a_dict[i]:
                # 如果y不在图中就先添加
                if j not in binpartiteGraph.get_y_Vertices():
                    binpartiteGraph.addVertex(j, 'y')
                binpartiteGraph.addUndirectedEdge(i, j)
        return binpartiteGraph
